...
**目前狀態：v0.1.0 開發中。核心流程通了，但還沒有發行版 —— 請先不要拿去營業。**

| 已經可用 | 進行中 |
| --- | --- |
| 商品維護（分類 / 品項 / 規格） | 班別日結（M6） |
| **點餐、加點、退點、結帳（含混合支付與找零）** | 發行硬化與安裝檔（M7） |
| **出單：網路型 ESC/POS、分區路由、佇列重試與補印** | KDS（v1.1） |
| 定價引擎（折扣 / 服務費 / 內含稅 / 抹零） | 掃碼點餐（v1.3） |
| 備份與還原、系統健康檢查 | 電子發票（v1.4） |

「點餐 → 收錢 → 找零 → 出單」現在是一條完整的路：一台網路型 ESC/POS
接上去、在設定頁按「測試列印」看得懂那張紙，就可以開始用了。
還沒做的是班別交接與日結，所以**目前還交不了班**。
...
| 版本 | 內容 |
| --- | --- |
| **v1.0** | 點餐 + 結帳 + 出單 + 班別日結 + 備份（點餐、結帳、出單、商品維護、備份已完成；還差班別日結與發行硬化） |
| v1.1 | KDS 廚房顯示 |
| v1.2 | 報表強化 |
| v1.3 | 顧客掃碼自助點餐 |
