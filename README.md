...
| 已經可用 | 進行中 |
| --- | --- |
| 商品維護（分類 / 品項 / 規格） | 出單機（M5） |
| **點餐、加點、退點、結帳（含混合支付與找零）** | 班別日結（M6） |
| 定價引擎（折扣 / 服務費 / 內含稅 / 抹零） | KDS（v1.1） |
| 收據與廚房單版面引擎 | 掃碼點餐（v1.3） |
| 備份與還原、系統健康檢查 | 電子發票（v1.4） |

目前結帳會把收據與廚房單寫進出單佇列，但**還沒有真的送到印表機** ——
那是 M5。所以現在可以完整跑一遍「點餐 → 收錢 → 找零」，只是印不出紙。
