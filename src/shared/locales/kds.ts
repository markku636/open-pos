...
export const kds = {
  ...
  // ★ 通路的字在這裡，不在後端。
  //
  //   後端給的是代碼（dine_in / takeout / delivery），因為它不知道現在站在
  //   螢幕前的是誰。以前那三個字是 Rust 裡寫死的中文，於是日文的廚房
  //   單頭永遠印著「外帶」—— 整面板只有這一格沒被翻到。
  //
  //   這三個詞不能再縮短：日本廚房講的就是「テイクアウト」，砍成「テイク」
  //   會像簡訊縮寫。所以是單頭去讓寬度（見 kds/App.tsx 的兩列抬頭），
  //   不是把詞砍到塞得下。
